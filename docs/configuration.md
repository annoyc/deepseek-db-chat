...
This document describes all configuration options for **DeepSeek DB Chat** — an AI-powered MySQL assistant that uses the DeepSeek API to translate natural language into SQL queries.
...
DeepSeek DB Chat supports two ways to provide an API key.
...
DeepSeek DB Chat supports two models:
...
DeepSeek DB Chat includes the inlined [deepseek-kit](https://github.com/FliPPeDround/deepseek-kit) core (`src/core/`) with several built-in optimizations to reduce API cost and improve reliability.
