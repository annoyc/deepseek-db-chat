...
DBPilot is an AI-powered MySQL assistant built on an inlined copy of **deepseek-kit** (`src/core/`). This document explains the key concepts behind the agent, tools, streaming, human-in-the-loop SQL safety, thinking mode, error handling, and data visualization.
...
DBPilot assembles the agent from three deepseek-kit primitives:
...
DBPilot registers three database tools in `src/server/tools.ts`:
...
DeepSeek models support an extended **thinking** mode that exposes the model's chain-of-thought as `reasoning_content` in streaming deltas. DBPilot enables this by default.
...
DBPilot inherits resilience mechanisms from the inlined deepseek-kit core. These operate at both the HTTP layer (API calls) and the tool layer (database operations).
