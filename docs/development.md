...
This guide covers local setup, project layout, key files, and conventions for contributing to **DBPilot** — an AI-powered MySQL assistant.
...
The `src/core/` directory contains an inlined copy of deepseek-kit rather than using it as an npm dependency. This allows direct control over the agent loop, streaming parser, and reasoning content handling — features that are central to the DBPilot experience.
