...
import { type IntentClassification, classifyIntentWithLLM, buildRoutingPrompt, FILTER_CONFIDENCE_THRESHOLD } from './intent-router'
...
interface AgentOptions {
  ...
  userQuery?: string
  /** Pre-computed intent classification. When provided, skips internal LLM call. */
...
7. 禁止文本提问代替交互控件：需要用户选择参数时，必须调用 smart_filter 工具，禁止以文本形式提问或自行假设参数值。
...
2b. 获取表结构后，如果发现查询涉及的表有日期字段或枚举字段，而用户未明确指定这些参数的值，应调用 smart_filter 让用户确认。调用后立即停止回复。
...
- 如果 plan_query 报告某些表不存在，必须修正计划后重新规划。
- 多步查询每步先执行一条 SQL，拿到结果后再决定下一步。`
...
用户问："查询最近创建的用户数"（多表可选 + 时间模糊）
   filters: [
     { type: "option_select", table: "", column: "", label: "查询范围", options: ["esmp_user", "sys_user", "全部"], defaultValue: "全部" },
     { type: "date_range", table: "esmp_user", column: "create_time", label: "创建时间", defaultRange: "30d" }
   ]

用户问："首页访问趋势"（表有日期字段但用户未指定范围和粒度）
   filters: [
     { type: "date_range", table: "page_visits", column: "visit_time", label: "时间范围", defaultRange: "30d" },
     { type: "aggregation", table: "page_visits", column: "visit_time", label: "统计粒度", aggregationOptions: ["按日","按周","按月"], defaultValue: "按日" }
   ]`
...
export async function createDbAgent(connection: DatabaseConnection, options?: AgentOptions) {
  ...
  const intent: IntentClassification | null = options?.intent !== undefined
    ? options.intent
    : userQuery
  ...
  if (intent) {
    ...
    if (intent.skipOverview) {
      systemPrompt += '\n无需调用 get_database_overview，直接基于已有上下文回答。'
    }
    if (intent.needsFilter >= FILTER_CONFIDENCE_THRESHOLD) {
      systemPrompt += '\n提示: 获取表结构后请先用 smart_filter 确认查询参数。'
    }
  }
