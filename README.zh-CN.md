...
敏感密码及密钥均使用 AES-256-GCM 加密存储。所有 SQL 查询经过**双重校验** — 工具层白名单拦截非查询语句，执行前再由你人工确认，两道关卡确保危险操作（INSERT / UPDATE / DELETE / DROP）无法执行，你的数据库永远不会被意外操作伤害。
...
- 🔒 **安全可靠** — AES-256 密码加密，SQL 双重校验（工具层白名单 + 人工确认），危险操作双门拦截
